...
describe("DiagnosticAggregator", () => {
  ...
  it("should handle all sources failing", async () => {
    const failing1: DiagnosticSource = {
      name: "failing1",
      isAvailable: async () => true,
      generate: async () => { throw new Error("fail1"); },
    };

    const failing2: DiagnosticSource = {
      name: "failing2",
      isAvailable: async () => true,
      generate: async () => { throw new Error("fail2"); },
    };
  ...
  it("should handle isAvailable throwing error", async () => {
    const errorSource: DiagnosticSource = {
      name: "errorSource",
      isAvailable: async () => { throw new Error("availability check failed"); },
      generate: async () => [{ range: { start: { line: 0, character: 0 }, end: { line: 0, character: 1 } }, message: "test", severity: 2 }],
    };
  ...
  it("should add and remove sources dynamically", async () => {
    const source1: DiagnosticSource = {
      name: "source1",
      isAvailable: async () => true,
      generate: async () => [{ range: { start: { line: 0, character: 0 }, end: { line: 0, character: 1 } }, message: "test1", severity: 2 }],
    };

    const source2: DiagnosticSource = {
      name: "source2",
      isAvailable: async () => true,
      generate: async () => [{ range: { start: { line: 0, character: 0 }, end: { line: 0, character: 1 } }, message: "test2", severity: 2 }],
    };

    const aggregator = new DiagnosticAggregator([source1]);

    // Initially one source
    let diagnostics = await aggregator.generate("file:///test.md", "content", "/project");
    assertEquals(diagnostics.length, 1);

    // Add second source
    aggregator.addSource(source2);
    diagnostics = await aggregator.generate("file:///test.md", "content", "/project");
    assertEquals(diagnostics.length, 2);
  ...
  it("should preserve diagnostic properties", async () => {
    const source: DiagnosticSource = {
      ...
      generate: async () => [{
        ...
        severity: 1,
        code: "TEST001",
        // source field will be set by aggregator
      }],
    };
