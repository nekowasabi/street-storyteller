...
import type { Timeline, TimelineEvent } from "../../../../src/type/v2/timeline.ts";
...
Deno.test("ElementEventCommand", async (t) => {
  ...
  await t.step("必須オプション（timeline, title, category, order, summary）の検証", async () => {
    const command = new ElementEventCommand();
  ...
  await t.step("有効なオプションでイベントがタイムラインに追加されること", async () => {
    const command = new ElementEventCommand();
...
`;
      await Deno.writeTextFile(`${timelineDir}/main_story.ts`, timelineContent);
  ...
  await t.step("caused-byとcausesオプションでイベント間の因果関係が設定できること", async () => {
    const command = new ElementEventCommand();
    const tempDir = await Deno.makeTempDir();
...
`;
  ...
  await t.step("存在しないタイムラインへのイベント追加はエラーになること", async () => {
    const command = new ElementEventCommand();
    const tempDir = await Deno.makeTempDir();
