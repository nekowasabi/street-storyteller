...
| サブプロットを追加 | `storyteller element subplot --name <id> --type subplot\|main\|parallel --summary "..."` |
| ビート (物語の節目) を追加 | `storyteller element beat --subplot <id> --title "..." --structure-position setup\|confrontation\|resolution` |
...
`src/characters/`, `src/settings/`, `src/timelines/`, `src/foreshadowings/`, `src/subplots/` 配下のファイルは TypeScript 型として記述します。`storyteller element ...` コマンドはこれらの型に整合する `.ts` ファイルを自動生成します。手動編集する場合は `@storyteller/types/v2/character.ts` 等の型をインポートしてください。
