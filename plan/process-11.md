...
- `internal/testkit/clock/fake_timer.go` — `(*FakeClock).AfterFunc` + `fakeTimer`/`fakeTimerHandle` 実装
- `internal/testkit/clock/timer_test.go` — `TestFakeTimerAfterFuncFiresAfterAdvance` 等 5 テスト
- `internal/testkit/lint_test.go` — `TestNoTimeSleepInDefaultTests` (AST walker, default-tag tests に `time.Sleep` 禁止)
- `internal/testkit/guard_test.go` — `TestNoOsExecImportInDefaultTests` (AST walker, default-tag tests に `os/exec` import 禁止)

### 変更ファイル
- `internal/testkit/clock/clock.go` — `FakeClock.pendingTimers` フィールド追加 + `Advance`/`Set` から `fireDueTimers()` 呼出
...
`internal/testkit/lint_test.go` と `guard_test.go` は `//go:build` を確認し、`integration`/`external` タグの付いたファイルは違反対象から除外する。

### 高速化見込み
TS版で 15分超かかっていた LSP E2E / textlint / file watching が、testkit を活用することで Go 版では 1 秒未満に短縮可能 (詳細は `docs/migration/go-rearchitecture-requirements.md`)。
