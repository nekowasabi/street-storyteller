...
| `view <setting\|timeline\|foreshadowing\|subplot>` | `modules/view` | 各エンティティ表示 |
...
```
character | setting | timeline | foreshadowing | subplot | beat | event | intersection | phase
```
...
| `subplot` | `--type main\|subplot\|parallel\|background` |
| `beat` | `--subplot <id>` `--title` `--summary` `--structure-position setup\|...` |
| `intersection` | `--source-subplot` `--source-beat` `--target-subplot` `--target-beat` `--summary` `--influence-direction forward\|backward` |
...
storyteller view subplot --id love_story --json
```
