...
このドキュメントは、Street
Storytellerで使用されるLLMシステムプロンプトの共通基盤を定義します。
全てのプラットフォーム（Claude Desktop、Claude
Code、Neovim）で共有されるコアコンセプトと原則を記載します。
...
Street
Storytellerは原稿中のエンティティ参照を検出し、定義との整合性を検証します。
...
```markdown
@hero は剣を抜いた。 @heroine と @hero は @royal_capital へ向かった。
```
...
```markdown
勇者は剣を抜いた。 → hero.ts (信頼度: 90%) 王都の城門前で出会った。 →
royal_capital.ts (信頼度: 85%)
```
