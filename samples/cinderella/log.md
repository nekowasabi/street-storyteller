...
- **エラー内容**: 生成されたファイルが `@storyteller/types/` を使用するが、実際のパスは `src/type/`
...
```json
{
  "mcpServers": {
    "storyteller": {
      ...
      "args": ["mcp", "start", "--stdio", "--path", "/home/takets/repos/street-storyteller/samples/cinderella"],
...
- **原因**: `src/cli/modules/index.ts` の `registerCoreModules()` で `createMcpDescriptor` が呼ばれていない
- **影響**: MCPサーバーをCLIから起動できない
- **回避策**: 直接 `src/cli/modules/mcp/mcp_server.ts` を実行するか、手動で登録を追加
...
インポートパスを `@storyteller/types/v2/character.ts` から相対パスまたは正しいエイリアスに変更
...
**プロジェクト**: street-storyteller v1.0
**テスト日**: 2025-12-15
