...
Root cause: MCP delegates to DiagnosticsGenerator.generate() which produces LSP-protocol-shaped diagnostics without confidence/entityId. CLI bypasses this and maps PositionedMatch directly to richer DiagnosticOutput.

Fix options: (a) MCP switch to CLI's direct-detection approach, or (b) extend DiagnosticsGenerator to optionally include confidence/entityId.
