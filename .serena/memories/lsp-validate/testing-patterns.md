...
- Uses createTestContext() helper with stub presenter (info/error/success
  arrays)
- LspValidateCommand accepts deps: { loadEntities, listMarkdownFiles } for DI

## Key Test Patterns

- Temp dirs: `Deno.makeTempDir({ prefix: "storyteller_..." })` with try/finally
  cleanup
...
- Stale .claude/worktrees/ blocks deno task test type-checking; cleanup with
  `git worktree remove --force`

## Regression

- Original 5 tests use `diagnostics: unknown[]` - backward compatible with type
  extensions
