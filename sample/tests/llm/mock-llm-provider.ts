...
export class MockLLMProvider implements LLMProvider {
  ...
  private initializeResponses(): void {
    ...
    this.testResponses.set("hero_personality_consistency", {
      ...
      confidence: 0.85,
      reasoning:
        "勇者アレクスの描写は概ね性格設定と一致しています。正義感の強さは村を出る決意や、エリーゼを助けようとする行動に表れています。天然さは「黎明」の真の価値を知らない点などに見られます。",
      suggestions: [
    ...
    this.testResponses.set("heroine_introduction", {
      ...
      confidence: 0.92,
      reasoning:
        "エリーゼの登場シーンは印象的です。「ちょっと、そこの人！」という気さくな呼びかけから始まり、自由奔放さが表現されています。高位の杖を持つ描写で天才性も示されています。",
      suggestions: [
    ...
    this.testResponses.set("meeting_naturalness", {
      ...
      confidence: 0.78,
      reasoning:
        "出会いから仲間になるまでの流れがやや急すぎます。初対面で即座に一緒に旅をする決定は、もう少し段階的な信頼構築が必要です。",
      suggestions: [
    ...
    this.testResponses.set("emotional_depth", {
      ...
      confidence: 0.80,
      reasoning:
        "勇者の旅立ちへの不安と決意は「今日という日が、ついに来たか...」という台詞に表れています。18年間過ごした部屋との別れのシーンも感情的です。",
      suggestions: [
    ...
    this.testResponses.set("setting_consistency", {
      ...
      confidence: 0.88,
      reasoning:
        "王都の描写は魔法と剣術の共存を適切に表現しています。勇者の剣と、エリーゼの魔法の杖が自然に共存し、冒険者ギルドの存在も世界観を支えています。",
      suggestions: [
    ...
    this.testResponses.set("atmosphere_building", {
      ...
      confidence: 0.75,
      reasoning:
        "冒険の始まりの期待感は表現されていますが、緊張感がやや不足しています。",
      suggestions: [
        "魔王復活の兆候をもう少し不穏に描写",
        "旅の危険性を示唆するエピソードを追加",
      ],
    ...
    this.testResponses.set("show_dont_tell", {
      ...
      confidence: 0.82,
      reasoning:
        "一部で説明的な文章が見られます。特にキャラクターの性格説明が直接的すぎる箇所があります。",
      suggestions: [
    ...
    this.testResponses.set("dialogue_naturalness", {
      ...
      confidence: 0.87,
      reasoning:
        "対話は概ね自然で、各キャラクターの個性が反映されています。エリーゼの自由奔放さ、アレクスの真面目さが台詞に表れています。",
      suggestions: [
    ...
    this.testResponses.set("chapter_opening", {
      ...
      confidence: 0.70,
      reasoning:
        "「勇者は朝日とともに目を覚ました」という始まりは、ファンタジーとしてはありきたりです。",
      suggestions: [
        "アクションシーンから始める",
        "謎めいた描写から始める",
        "印象的な対話から始める",
    ...
    this.testResponses.set("foreshadowing", {
      ...
      confidence: 0.83,
      reasoning:
        "伏線は適切に配置されています。黎明の剣の真の価値、勇者の出自の秘密、魔王復活の兆候が自然に示されています。",
      suggestions: [
    ...
    this.testResponses.set("engagement", {
      ...
      confidence: 0.88,
      reasoning:
        "章の終わりで魔法の森への言及があり、次章への期待を高めています。キャラクター間の関係性も読者の興味を引きます。",
      suggestions: [
    ...
    this.testResponses.set("theme_introduction", {
      ...
      confidence: 0.81,
      reasoning:
        "成長と友情のテーマが自然に導入されています。説教臭さはなく、物語の中で自然に表現されています。",
      suggestions: [],
    });
    ...
    this.testResponses.set("chapter_quality", {
      ...
      confidence: 0.77,
      reasoning:
        "第1章として必要な要素は揃っていますが、いくつかの改善点があります。キャラクターは魅力的で、世界観も明確です。",
      suggestions: [
        "冒頭をもっとインパクトのあるものに変更",
